...
public class VerifyCodeUtils {
 ...
 // 使用到Algerian字体，系统里没有的话需要安装字体，字体只显示大写，去掉了1,0,i,o几个容易混淆的字符  
     ...
     * @return 图片缓存对象
     */
    public static BufferedImage generateImageCode(String code, int w, int h, int interLine,
...
//        ImageIO.write(image, "jpg", os);  
        ...
        for (int i = 0; i < code.length(); i++) {
            fy = randomLocation ? (int) ((Math.random() * 0.3 + 0.6) * h) : fy;// 每个字符高低是否随机
            g2.setColor(foreColor == null ? getRandomColor() : foreColor);
            g2.drawString(code.charAt(i) + "", fx, fy);
            fx += fsize * 0.9;
        }
