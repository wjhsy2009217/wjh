package com.hzgc.common.utils.poi;
...
public class ExcelUtil<T>
{
     ...
     * @return 结果
     */
    public AjaxResult exportExcel(List<T> list,String dir, String sheetName)
    {
        ...
        try
        {
            ...
            for (int index = 0; index <= sheetNo; index++)
            {
                ...
                for (int i = 0; i < fields.size(); i++)
                {
                    ...
                    {
                        // 这里默认设了2-101列提示.
                        setHSSFPrompt1(sheet, "", attr.prompt(), 1, 100, i, i);
                    }
                    // 如果设置了combo属性则本列只能选择不能输入
                    if (attr.combo().length > 0)
                    {
                        // 这里默认设了2-101列只能选择不能输入.
                        setHSSFValidation1(sheet, attr.combo(), 1, 100, i, i);
                    }
                }
            ...
            String filename = encodingFilenameTime(sheetName);
            out = new FileOutputStream(getAbsoluteFilePath(dir, filename));
            workbook.write(out);
    ...
    public static XSSFSheet setHSSFPrompt1(XSSFSheet sheet, String promptTitle, String promptContent, int firstRow,
                                           int endRow, int firstCol, int endCol)
    {
        // 构造constraint对象
        DVConstraint constraint = DVConstraint.createCustomFormulaConstraint("DD1");
    ...
    public static XSSFSheet setHSSFValidation1(XSSFSheet sheet, String[] textlist, int firstRow, int endRow,
                                               int firstCol, int endCol)
    {
        // 加载下拉列表内容
        DVConstraint constraint = DVConstraint.createExplicitListConstraint(textlist);
     ...
     * 编码文件名
     */
    public String encodingFilenameTime(String filename)
    {
//        filename = UUID.randomUUID().toString() + "_" + filename + ".xls";
    ...
    {
        String downloadPath = RuoYiConfig.getDownloadPath() + dir+ "/" + filename;
        File desc = new File(downloadPath);
        if (!desc.getParentFile().exists())
        {
            desc.getParentFile().mkdirs();
    ...
    public AjaxResult exportCSV(List<T> list, String dir, String filename)  {
        try {
            String downloadPath = RuoYiConfig.getDownloadPath() + dir+ "/" + filename + "_" +System.currentTimeMillis() + ".csv";

            CsvWriter writer = CsvUtil.getWriter(downloadPath, CharsetUtil.CHARSET_GBK);
            ...
            for (int i = 0; i < list.size(); i++) {
                T vo = (T) list.get(i);
                String entity[] = new String[fields.size()];
                for (int j = 0; j < fields.size(); j++) {
                    // 获得field.
                    Field field = fields.get(j);
                    ...
                    entity[j] = Convert.toStr(StringUtils.isNull(field.get(vo)) ? attr.defaultValue() : field.get(vo) + attr.suffix());
                }
                arr.add(entity);
            }
