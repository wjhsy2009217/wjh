package com.hzgc.framework.web.controller;
...
import com.hzgc.framework.web.page.TableSupport;
import com.hzgc.project.common.PageParam;
...
public class BaseController
{
     ...
     * Es设置请求分页数据
     */
    protected PageParam startPageEs()
    {
        PageDomain pageDomain = TableSupport.buildPageRequest();
        ...
        {
            return  PageParam.builder().setStart(pageNum).setLimit(pageSize);
        }
        String orderByColumn = pageDomain.getOrderByColumn();
        String isAsc = pageDomain.getIsAsc();
        if(isAsc.equals("desc")){
            return  PageParam.builder().setStart(pageNum).setLimit(pageSize).setDesc(true);
        }
        return  PageParam.builder().setStart(pageNum).setLimit(pageSize).setDesc(false);
    }
    ...
    protected TableDataInfo getDataTable(List<?> list, long totalElements)
    {
        ...
        rspData.setCode(0);
        rspData.setRows(list);
        rspData.setTotal(Convert.toInt(totalElements));
        return rspData;
    }
}
