...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            doExportPassingPicture: function (formId){
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                $.modal.loading("正在导出绊线图片，请稍后...");
                $.post($.table._option.exportPassingPictureUrl, $("#" + currentId).serializeArray(), function (result) {
                    if (result.code == web_status.FAIL) {
                        $.modal.alertError(result.msg);
                    }
                    $.modal.closeLoading();
            ...
            doExportPicture: function (formId){
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                $.modal.loading("正在导出图片，请稍后...");
                $.post($.table._option.exportPictureUrl, $("#" + currentId).serializeArray(), function (result) {
                    if (result.code == web_status.FAIL) {
                        $.modal.alertError(result.msg);
                    }
                    $.modal.closeLoading();
            ...
            // 仅解析
            analy: function (formId) {
                var currentId = $.common.isEmpty(formId) ? $('form').attr('id') : formId;
                var params = $("#bootstrap-table").bootstrapTable('getOptions');
                $.modal.loading("正在导出数据，请稍后...");
                $.post($.table._option.analyUrl, $("#" + currentId).serializeArray(), function (result) {
                    if (result.code == web_status.FAIL) {
                        $.modal.alertError(result.msg);
                    }
                    $.modal.closeLoading();
        ...
        operate: {
            ...
            // 详细信息
            macImport: function (id) {

                var _url = $.common.isEmpty(id) ? $.table._option.macImportUrl : $.table._option.macImportUrl.replace("{id}", id);
                ...
                layer.open({
                    ...
                    maxmin: true,
                    shade: 0.3,
                    title: $.table._option.modalName + "导入",
                    content: _url,
                    btn: ['<i class="fa fa-close"></i> 关闭'],
                    // 弹层外区域关闭
                    shadeClose: true,
